...
std::optional<std::string_view> TlsSniffer::ParseClientHello(std::span<const uint8_t> data) {
...
std::optional<std::string_view> TlsSniffer::ExtractSNI(std::span<const uint8_t> extensions) {
    size_t pos = 0;
        ...
        if (ext_type == tls::EXTENSION_SNI) {
            ...
            while (sni_pos + 3 <= sni_end) {
                ...
                if (name_type == tls::SNI_HOST_NAME) {
                    // 提取主机名视图，调用方只在最终 SniffResult 中复制一次。
                    ...
                    // 验证主机名格式
                    if (!hostname.empty() &&
                        std::memchr(hostname.data(), '\0', hostname.size()) == nullptr) {
                        return hostname;
                    }
                }
