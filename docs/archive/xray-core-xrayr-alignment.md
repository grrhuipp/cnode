...
- 当前进展：
  ...
  - Panel 动态 inbound 不再通过 `fixed_outbound` 绕过 Router；面板创建的 freedom outbound 只作为普通 outbound tag 存在，所有静态和动态 inbound 均先进入 `routing.json` 规则匹配。新增 `no_panel_inbound_fixed_outbound` 结构门防止回退。
