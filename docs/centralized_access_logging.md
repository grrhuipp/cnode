...
6. 面板 `Key` 不进入日志事件、不进入待发送批次、不发送给日志服务。
7. ClickHouse 用于明细、聚合和用户画像，不替代现有面板流量上报链路。
8. `log.enable` 默认为 `false`，默认不上报结构化日志且不启动上报线程；只有显式设为 `true` 才开启上报。本地文本日志继续正常写入。无论该开关取值如何，上报器都不创建磁盘 spool。

原 `log.disableUpload` 字段已移除，旧配置会在加载时明确报错。迁移时，`disableUpload: true` 改为 `enable: false`，`disableUpload: false` 改为 `enable: true`；省略开关时按新默认值关闭上传。
...
| 热路径事件队列容量 | `8192` 条事件 |
| 单流待发送批次内存 | `32 MiB`，access/error 各一条，超限删除最旧批次 |
| 单个压缩批次上限 | `8 MiB` |
| 建连超时 | `3 s` |
| 单次 I/O 超时 | `10 s` |
| 最大重试间隔 | `60 s`，指数退避并加抖动 |

待发送批次只存在于 reporter 线程拥有的进程内存中，access/error 两条流合计硬上限为 `64 MiB`。`Log.LogDir` 只控制本地 access/error 文本日志；上报器不在其中创建 `access-spool`、`error-spool`、`.batch` 或临时文件。进程退出或崩溃时，未确认批次不会跨重启恢复。
...
- 在 Worker 做 HTTP、TLS、压缩、待发送批次管理或 ClickHouse 操作。
- 阻塞等待队列空间。
...
- 批量编码和 zstd 压缩。
- 将压缩 payload 放入有界进程内存队列，再发送 HTTPS 请求。
- 按 ACK 释放已确认批次；重试期间复用同一 payload 和 batch ID。
- 连接复用、超时、重试、退避和指标统计。
...
| HTTP 状态 | 含义 | cnode 行为 |
| --- | --- | --- |
| `200` | 已可靠接收 | 释放已确认的内存批次 |
| `400` | schema 或字段非法 | 丢弃坏批次，不阻塞后续批次 |
...
5. cnode 收到 ACK 后释放内存批次。

超时可能发生在服务端已接收但客户端未收到 ACK 的时刻，因此重试会产生重复。`batch_id`、`event_id` 和 `(server_id, boot_id, sequence)` 在重试时必须保持不变；日志服务负责幂等接收或去重。

重试采用指数退避和随机抖动，建议从 500 ms 增长到最多 60 s。认证失败不得高频重试；服务端以 `400` 永久拒绝的坏批次直接丢弃，不能阻塞后续批次。

该语义不跨进程重启：正常退出、崩溃或重启都会丢弃尚未 ACK 的批次，因此不承诺跨重启至少一次。热路径事件队列满时绝不阻塞 Worker；任一待发送批次队列达到 `32 MiB` 上限后，删除该流最旧批次以保留最近故障证据，同时累计永久丢弃计数并告警。无论日志链路发生什么故障，代理主链路都继续工作。
...
待发送批次内存达到 70%/90%、出现永久丢弃、最旧未确认事件持续增长、ClickHouse 写入持续失败或某节点日志量归零时必须告警。
...
- 实现内存批次队列、ACK、重试、坏批次丢弃和容量策略。
- 用 mock 服务测试断网、超时、401、413、429、5xx、重复 ACK、内存上限和关闭丢弃。
...
只有多实例需要共享持久积压、ClickHouse 长时间维护仍需持续接收、峰值超过本地内存队列能力，或多个独立消费者需要同一事件流时，才在日志服务后增加 Kafka。

## 17. 验收标准

1. 每条事件可准确定位 `panel APIHost + NodeType + NodeID + user_id`。
2. 面板密钥、代理凭据和日志服务密钥不出现在事件、待发送批次和 ClickHouse。
3. 每个连接最多提交一条规范终态事件，重试保持同一 `event_id`。
4. 日志服务不可用时代理主链路无可测退化，事件进入有界内存批次队列。
5. 同一进程内服务恢复后自动补传；跨重启不承诺恢复未确认批次。
6. 热路径队列或待发送批次内存满时代理继续工作，永久丢弃数量可观测。
7. 能按面板、节点、用户和时间查询流量、连接数、失败率和目标地址。
8. Worker 不持有日志 HTTP 客户端、ClickHouse 驱动或面板原始配置。
9. 跨线程事件不引用 Worker-local 对象。
10. 断网、超时、认证失败、限流、重复批次、坏 schema、内存上限和关闭丢弃均有自动化测试。
